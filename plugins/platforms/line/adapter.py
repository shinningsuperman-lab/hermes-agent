...
GROUP_CHIME_COORDINATION_PATTERNS: Tuple[re.Pattern[str], ...] = tuple(
    ...
    for pattern in (
        ...
        r"(?:\d{1,2}\s*(?:/|月|號)|週[一二三四五六日天]).{0,40}(?:不能打|沒辦法打|無法打|不能去|請假|臨時(?:插入|有|加).{0,12}(?:同學會|會議|行程))",
        r"(?:不能打|沒辦法打|無法打|不能去|請假|臨時(?:插入|有|加).{0,12}(?:同學會|會議|行程)).{0,40}(?:\d{1,2}\s*(?:/|月|號)|週[一二三四五六日天]|中午|早上|下午|晚上)",
    )
)
